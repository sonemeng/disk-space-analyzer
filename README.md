...
| 🧩 **注册表检查** | 检查用户级失效启动项、App Paths 和卸载残留，强制备份后修复低风险项 |
...
- 检查当前用户范围内目标已不存在的启动项与 App Paths。
- 对安装目录和卸载程序均不存在的卸载信息只给出人工复核建议。
- 修复前强制通过 Windows `reg export` 导出涉及分支；任一备份失败则不修改注册表。
- 不扫描或清理系统服务、驱动、COM、文件关联和共享 DLL，不需要管理员权限。
